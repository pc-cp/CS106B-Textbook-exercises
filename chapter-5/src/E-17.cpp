 ...
 *
 * there is a single waiting line served by multiple cashiers
 */

void runSimulation(int &nServed, int &totalWait, int &totalLength)
{
    Queue<int> queue;
    int timeRemaining[2] = {0, 0};
    nServed = 0;
    totalWait = 0;
    totalLength = 0;
    for(int t = 0; t < SIMULATION_TIME; t++)
    {
        ...
        else
        {
            if (!queue.isEmpty())
            {
                ...
                nServed ++;
                timeRemaining[0] = randomInteger(MIN_SERVICE_TIME, MAX_SERVICE_TIME);
            }
        ...
        else
        {
            if (!queue.isEmpty())
            {
                totalWait += t - queue.dequeue();
                nServed ++;
                timeRemaining[1] = randomInteger(MIN_SERVICE_TIME, MAX_SERVICE_TIME);
            }
