 ...
 * 实现笔记：isBalanced
 * ------------------
 * 如何使得每个结点仅遍历一次的条件下完成这个函数
 * 首先判断左右子树是否平衡，若平衡再判断根结点是否平衡
 */
template <typename ValueType>
bool isBalanced(BSTNode<ValueType> *tree) {
    if(tree == nullptr) {
        return true;
    }
    // 记忆化搜索，使得每个结点仅需遍历一次，无需为求树的高度单独遍历
    MyMap<ValueType, int> keyToHeight;
    return isBalancedRec(tree, keyToHeight);
}

template <typename ValueType>
bool isBalancedRec(BSTNode<ValueType> *tree, MyMap<ValueType, int> &keyToHeight) {
    if(tree == nullptr) {
        return true;
    }
    else {
        ...
        if(tree->left) {
            leftBalanced = isBalancedRec(tree->left, keyToHeight);
            leftHeight = keyToHeight[tree->left->key];
        }
        if(tree->right) {
            rightBalanced = isBalancedRec(tree->right, keyToHeight);
            rightHeight = keyToHeight[tree->right->key];
        }

        keyToHeight[tree->key] = std::max(leftHeight, rightHeight) + 1;
        return leftBalanced && rightBalanced && (std::abs(rightHeight-leftHeight) <= 1);
    }
}

/*
 * Ex-16-8
 * 实现笔记：hasBinarySearchProperty
 * -------------------------------
 * 判断一棵树是否为BST，中序遍历后判断
 * 是否有序即可，因为BST要求中序遍历输出有序
 */
template <typename ValueType>
bool hasBinarySearchProperty(BSTNode<ValueType> *tree) {
    std::vector<ValueType> vec;
    hasBinarySearchPropertyRec(tree, vec);

    bool inverseAndEqual = false;
    for(int i = 0; i < vec.size()-1; ++i) {
        if(vec[i] >= vec[i+1]) {
            inverseAndEqual = true;
        }
    }
    return !inverseAndEqual;
}

template <typename ValueType>
void hasBinarySearchPropertyRec(BSTNode<ValueType> *tree, std::vector<ValueType> &vec) {
    if(tree == nullptr) {
        return ;
    }

    hasBinarySearchPropertyRec(tree->left, vec);
    vec.push_back(tree->key);
    hasBinarySearchPropertyRec(tree->right, vec);
}
