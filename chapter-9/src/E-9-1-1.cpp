...
int main() {
    ...
    if(length == NO_PATH) {
        ...
        cout << "Shortest path through maze = " << length << endl;
    }
    return 0;
}
...
int shortestPathLength(Maze &maze, Point start) {
    ...
    if(solveLengths.size()) {
        // cout << solveLengths << endl;
        return solveLengths.first();
    }
    else {
        return NO_PATH;
    }
 ...
 * Usage: if(findSolutionLength(maze, start, length, solveLengths)) ...
 * ---------------------------------------------------------------------
 * Find the various solution paths through a maze and returns the corresponding
...
bool findSolutionLengths(Maze &maze, Point start, int &length, Set<int> &solveLengths) {
    ...
    for(Direction dir = NORTH; dir <= WEST; dir++) {
        if(!maze.wallExists(start, dir)) {
            if(findSolutionLengths(maze, adjacentPoint(start, dir), length, solveLengths)) {
