 ...
 * key is found, the function returns the index in the vector at which
 * that key appear. (If the key appears more than once in the vector,
 * any of the matching indices may be returned). If the key does not exist in the vector,
 * the function returns -1. This implementation is simply a wrapper function; all of the
 * real work is done by the more general binarySearch function.
 */
int findInSortedVector(string key, Vector<string> &vec)
{
...
int binarySearch(string key, Vector<string> &vec, int p1, int p2)
{
    ...
    if(key < vec[mid])  return binarySearch(key, vec, p1, mid-1);
    else return binarySearch(key, vec, mid+1, p2);
}
